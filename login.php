...
if ( isset($_POST["submit"]) ) {
    // If form submitted, get values from the database.
    require("db.php");
    ...
    if ( empty($errors) ) {
        ...
        try {
            $result = $db->querySingle("SELECT * FROM users WHERE email='$email' AND password='" . md5($password) . "';");
            if ($result) {
                ...
                $errors["failed"] = "Invalid E-mail or password";
            };
        } catch (Exception $e) {
            // echo $e;
...
<head>
    ...
    <title>Login - Pirate Bank 2</title>
    <style>
        body {
            margin: 0;
            text-align: center;
...
<body>
    <div>
        ...
        <h2>Login</h2>
        <?php
            if ( empty($errors) && isset($_SERVER["HTTP_REFERER"]) && isset($_GET["status"]) && $_GET["status"] === "success" ) {
